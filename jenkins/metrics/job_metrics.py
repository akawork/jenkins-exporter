...
def make_metrics(jobs):
    ...
    # Total job
    metric = GaugeMetricFamily('jenkins_jobs_total',
    ...
    metric = None
    metric = GaugeMetricFamily('jenkins_job_builds_total',
                                        'Total builds of a job',
                                        labels=['job_name'])
    for job_id in list_jobs: # job_id == job['full_name']
        metric.add_metric(labels=[job_id], 
    ...
    metric = None
    metric = GaugeMetricFamily('jenkins_job_fail_consecutively',
                            'The number of times the last consecutive' \
                                + 'failure build of a job',
                            labels=['job_name'])
    ...
    # Total building jobs
    metric = None
    metric = GaugeMetricFamily('jenkins_jobs_building_total',
                                'Total building jobs',
                                labels=None)
    metric.add_metric(labels=[], 
    ...
    metric = None
    metric = GaugeMetricFamily('jenkins_jobs_building_time_seconds',
    ...
    for job_id in list_jobs:
        ...
        if building_duration > 0:
            metric.add_metric(labels=[job_id, 'building'], 
                            value=building_duration)
        else:
            if jobs.get_total_builds(job_id) == 0:
                metric.add_metric(labels=[job_id, 'not_yet_built'], 
                                    value=0)
            else:
                metric.add_metric(labels=[job_id, 'not_building'], 
                                    value=0)
